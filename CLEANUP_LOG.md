...
**Stav k poslední aktualizaci**: Fáze 0 (baseline) hotová, Priorita 1 (mrtvý kód) uzavřena, Priorita 2 (Firestore náklady) uzavřena, Priorita 3 (rebuild performance) rozpracovaná — 7 atomických změn commitnutých celkem (4 z nich v Prioritě 3: #4, #5, #6, #7), viz tabulka níže. **Další krok: pokračovat v Prioritě 3** — audit nálezů a pořadí viz sekce "Priorita 3 — Rebuild performance" níže. Zbývají nálezy #1 (`cycle_tracking_screen.dart`, velký/rizikový), #4 (`intimacy_history_list.dart`), #5 (`data_screen.dart`), #6 (`timeline_screen.dart` regroup), #8 (`timeline_screen.dart` MemoryCard setState). Nález #2 (`home_screen.dart`) je odložený/rizikový, viz poznámka u něj — nedělat bez výslovné domluvy.
