...
**Stav k poslední aktualizaci**: Fáze 0 (baseline) hotová, Priorita 1 (mrtvý kód) uzavřena, Priorita 2 (Firestore náklady) uzavřena, Priorita 3 (rebuild performance) rozpracovaná — 11 atomických změn commitnutých celkem (8 z nich v Prioritě 3: #4–#11), viz tabulka níže. **Firestore composite index nasazen do produkce** (2026-08-08) — akce z Fáze 0 už není blokující. **Další krok: pokračovat v Prioritě 3** — audit nálezů a pořadí viz sekce "Priorita 3 — Rebuild performance" níže. Zbývají nálezy #1 (`cycle_tracking_screen.dart`, velký/rizikový, chce samostatnou domluvu o rozsahu), zbytek nálezu #4 (`currentUserDataProvider`/`partnerProvider` + group-by-month sort v `intimacy_history_list.dart`), #6 (`timeline_screen.dart` regroup na XP/premium watch — vědomě přeskočeno, viz poznámka u něj, potřebovalo by extrakci `_TimelineList` do vlastního provider-watching widgetu). Nález #2 (`home_screen.dart`) je odložený/rizikový, viz poznámka u něj — nedělat bez výslovné domluvy.
