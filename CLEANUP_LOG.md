...
**Stav k poslední aktualizaci**: Fáze 0 (baseline) hotová, Priorita 1 (mrtvý kód) uzavřena, Priorita 2 (Firestore náklady) uzavřena, Priorita 3 (rebuild performance) rozpracovaná — 9 atomických změn commitnutých celkem (6 z nich v Prioritě 3: #4–#9), viz tabulka níže. **Další krok: pokračovat v Prioritě 3** — audit nálezů a pořadí viz sekce "Priorita 3 — Rebuild performance" níže. Zbývají nálezy #1 (`cycle_tracking_screen.dart`, velký/rizikový), zbytek nálezu #4 (`currentUserDataProvider`/`partnerProvider` + group-by-month sort), #5 (`data_screen.dart`), #6 (`timeline_screen.dart` regroup), #8 (`timeline_screen.dart` MemoryCard setState). Nález #2 (`home_screen.dart`) je odložený/rizikový, viz poznámka u něj — nedělat bez výslovné domluvy.
