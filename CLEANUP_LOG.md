...
**Stav k poslední aktualizaci**: Fáze 0 (baseline) hotová, Priorita 1 (mrtvý kód) uzavřena, Priorita 2 (Firestore náklady) uzavřena, Priorita 3 (rebuild performance) z větší části uzavřena (zbylé nálezy vědomě odložené), **Priorita 4 (duplicita) uzavřena**, Priorita 5 (struktura) rozpracovaná — 18 atomických změn commitnutých celkem, viz tabulka níže. **Firestore composite index nasazen do produkce** (2026-08-08) — akce z Fáze 0 už není blokující. **Další krok: pokračovat v Prioritě 5** — `add_memory_screen.dart` (884 řádků), `app_router.dart` (853, vyšší riziko kvůli routing-table soudržnosti) jsou další kandidáti (mimo `add_intimacy_sheet.dart`, které se nesmí dotknout). `cycle_tracking_screen.dart` (680) a `timeline_screen.dart` (405) už jsou pod 400řádkovou hranicí prakticky/blízko ní po changes #15/#17/#18.
...
| 17 | `_DayOptionsWidget` (day-detail karta s memories/intimacy/events sekcemi a quick-add tlačítky, ~380 řádků) brala všechna data přes konstruktor, žádný sdílený stav s obrazovkou — vytažena do `day_options_widget.dart`, přejmenována na veřejnou `DayOptionsWidget`. Čistý přesun. Vedlejší úklid: 6 importů (`add_event_sheet`, `add_intimacy_sheet` [jen import, soubor samotný nedotčen], `intimacy_log_detail_sheet`, `memory_detail_dialog`, `go_router`, plus `phosphor_flutter`/`intl`/`bento_card`/`time_format`) se v `cycle_tracking_screen.dart` staly nepoužitými, odstraněny. Ověřeno `flutter build linux --debug` + analyze/test beze změny. `cycle_tracking_screen.dart`: 1061→680 řádků (z původních 1373 na začátku Priority 3). | `lib/features/cycle/presentation/day_options_widget.dart` (nový), `lib/features/cycle/presentation/cycle_tracking_screen.dart` | 387 issues (beze změny) | 153/153 ✅ | commit `da9df3d` |
| 18 | `MemoryCard` (StatefulWidget karta paměti s image PageView/page-dots, ~275 řádků) + jeho privátní pomocníci `_MemoryImage`, `_PageIndicator` a `_MemoryCardSkeleton` (bez polí, jen konstruktor) brali data jen přes konstruktor, žádný sdílený stav s `TimelineScreen` — vytaženy do `widgets/memory_card.dart`. `MemoryCard` byl už veřejný (beze změny), `_MemoryCardSkeleton` přejmenován na veřejný `MemoryCardSkeleton` (referencovaný z `_LoadingState` v `timeline_screen.dart`) + dostal `super.key` (nový `use_key_in_widget_constructors` lint po zveřejnění, stejný vzor jako change #16). Čistý přesun, žádná změna logiky. Vedlejší úklid: importy `cached_network_image`/`memory_detail_dialog` se v `timeline_screen.dart` staly nepoužitými, odstraněny. Ověřeno `flutter build linux --debug` + analyze/test beze změny (387/153, potvrzeno i porovnáním se stashnutým stavem). `timeline_screen.dart`: 829→405 řádků. | `lib/features/timeline/presentation/widgets/memory_card.dart` (nový), `lib/features/timeline/presentation/screens/timeline_screen.dart` | 387 issues (beze změny) | 153/153 ✅ | commit `5e95c07` |
