...
**Stav k poslední aktualizaci**: Fáze 0 (baseline) hotová, Priorita 1 (mrtvý kód) uzavřena, Priorita 2 (Firestore náklady) uzavřena, Priorita 3 (rebuild performance) rozpracovaná — 5 atomických změn commitnutých celkem (2 z nich v Prioritě 3: #4 a #5), viz tabulka níže. **Další krok: pokračovat v Prioritě 3** — audit nálezů a pořadí viz sekce "Priorita 3 — Rebuild performance" níže. Nález #2 (`home_screen.dart`) je odložený/rizikový, viz poznámka u něj — pokračovat nálezem #3 nebo #10 (menší, izolovanější).
