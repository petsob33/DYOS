...
#### Aplikační runtime
1. `main()` v `lib/main.dart` inicializuje Firebase, registruje FCM background handler a podmíněně inicializuje RevenueCat přes `REVENUECAT_API_KEY`.
...
- `PairingScreen._submitCode()` (`lib/features/auth/presentation/pairing_screen.dart`) volá:
  - `FirebaseService.findUserByInviteCode()`,
  - `FirebaseService.pairUsers(currentUser.uid, partnerUser.uid)`.
- `pairUsers()` (`lib/core/services/firebase_service.dart`) vytvoří `couples/{coupleId}` + batch update obou `users/{uid}.coupleId`.
...
- `FirebaseService.pairUsers()` - klíčová doménová operace párování.
...
### Kritické
1. **Hardcoded Google Maps API key v Android manifestu**
...
2. **Pravidlo `users/{userId}` umožňuje nepřímou eskalaci přes `coupleId`**
...
3. **Cloud Function vrací celé user dokumenty podle invite code**
...
### Vysoké
4. **Nechráněný soubor secrets v repu policy**
...
5. **Nedostatečná validace vstupu ve Function `getUserByInviteCode`**
...
6. **Párování není transakčně odolné proti závodu**
...
### Střední
7. **Příliš široké oprávnění čtení profilových obrázků**
...
8. **Rozsáhlé logování interních dat**
...
- [ ] Odstranit hardcoded Android Maps key z `AndroidManifest.xml`, rotovat klíč a omezit v GCP.
- [ ] Opravit `firestore.rules` pro `users/{userId}` (zákaz cizího zápisu `coupleId` bez serverově ověřené pozvánky).
- [ ] Upravit Cloud Function `getUserByInviteCode` na minimální response payload + vstupní validace.
- [ ] Přidat `ios/Runner/Secrets.xcconfig` do `.gitignore` a zavést bezpečný secrets workflow.
- [ ] Přesunout pairing logiku do atomické server-side transakce (Cloud Function).

### Vysoká priorita před produkcí
- [ ] Zúžit read přístup v `storage.rules` pro `profile_pictures`.
- [ ] Zavést rate limiting/App Check pro callable endpointy.
- [ ] Odstranit debug route `/firebase-test` z release (router už ji váže na `kDebugMode`, ověřit release build pipeline).
