...
- Aktuální kód je čistý (`Info.plist` používá `$(GOOGLE_PLACES_API_KEY)` placeholder), ale stejný reálný klíč (`AIzaSyAIFphUOhpjty5H7AcXoEjWBUGqfnzXV_A`) byl v minulosti hardcoded v `AppDelegate.swift` (commit `67e9f1e`) a je stále dohledatelný přes `git log -p`.
